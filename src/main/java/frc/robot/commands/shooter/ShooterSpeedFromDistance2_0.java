...
public class ShooterSpeedFromDistance2_0 extends CommandBase {
  ...
  private double distAverage = 0;
  private double averageConstant = 0.75;
  // const * newVal + 1 - const * oldValue
  ...
  public ShooterSpeedFromDistance2_0(Shooter m_shooter) {
    shooter = m_shooter;
  ...
  public void initialize() {
    ...
    distAverage = shooter.estimateDistance(ty.getNumber(0).doubleValue()) * 0.9438;
    shooter.setPID(
  ...
  public void execute() {
    //if (!setSpeed) {
      ...
      distance = (averageConstant * distance) + ((1 - averageConstant) * distAverage);
      distAverage = distance;
  ...
  public boolean isFinished() {
    ...
    System.out.println(rpms);
    if (counter > 13 && 
    (rpms-errorMargin) < shooter.getBottomShooterSpeed() && shooter.getBottomShooterSpeed() < (rpms+errorMargin) &&
    (rpms-errorMargin) < shooter.getTopShooterSpeed() && shooter.getTopShooterSpeed() < (rpms+errorMargin)
    ) {
      return true;
    } else if (
      (rpms-errorMargin) < shooter.getBottomShooterSpeed() && shooter.getBottomShooterSpeed() < (rpms+errorMargin) &&
